...
import trevor, { speechSynth } from './Speech/SpeechSynth';
...
const List = ({ history }) => {
  ...
  const handleSubmit = async event => {
    if (ingredient === '') {
      trevor.text = `you need to add an item`;
      speechSynth.speak(trevor);
    }
          ...
          <div className="showcase container">
            <div className="row">
              <div className="col s12 m10 offset-m1 center">
                ...
                <input
                  value={ingredient} name="name" onChange={handleChange} required />
              </div>
              <button
              ...
              >
                Add Ingredient
							...
							<button onClick={startAnnyang}>Start Listening!</button>
            </div>
          </div>
        </div>
      </form>
